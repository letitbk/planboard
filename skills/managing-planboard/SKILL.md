...
- If execution deviated materially from the plan, propose `v<N+1>.md` with a `Supersedes` line stating what changed and why. `/research-plans:sync` records the confirmed amendment. Re-execution recommits it through a sign session first.
...
- `/research-plans:sync` is the manual recovery checkpoint; the primary loop is plan → draft review → execute gate → tail. Use sync's late-capture protocol when logging was missed outside the loop.
