...
const comment1: StoredComment = {
  ...
  annotation: { type: "doc-comment" }, receivedAt: "2026-01-01T00:00:00.000Z",
};
...
describe("putComment", () => {
  it("calls put() with the comments/<id>.json pathname, private access, allowOverwrite, and the token", async () => {
    await putComment(TOKEN, comment1);
    ...
    expect(options).toMatchObject({ access: "private", allowOverwrite: true, token: TOKEN });
  });
});
