...
| `/research-plans:board` | Browser board: tracker (with drift flags), plans + diffs, results, reports, timeline, scorecards; persistent live control surface (stable port, relaunch loop) with always-on approve/request-changes/review actions, one-click agent review (Codex/Gemini/subagent panel), or static export |
