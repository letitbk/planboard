...
- Results capture runs automatically as part of `/research-plans:sync` for each component that moved to `done` and for captured sources that drifted. The per-component `/research-plans:results` interview still runs and the researcher confirms every bundle, so capture stays visible and evidence-based, never a silent bulk write.
