...
6. **Sign-off.** Read the finished draft back in brief. A board preview is still available during the dialogue (write the draft to `plans/execution/<NN-slug>/.draft-v1.md` and run `/research-plans:board <NN-slug>`; fold annotations into the draft), but **formal approval happens at the sign-off gate**: writing `v1.md` automatically opens the plan in the researcher's browser, where they approve or request changes. Before writing, make sure any preview board session has been submitted or closed — the gate needs the board. Add the sign-off line with the researcher's name and today's date, then write `v1.md`. If the gate denies, read `plans/.board-feedback.md`, revise the draft to address all feedback, and write again (the gate reopens). After approval lands: delete the draft file if one exists, update the component's tracker row to `planned` with a link to the plan, and update `Last updated:`.
